...
public class SessionRegistry {
  private final Map<String, WaitingRoom> roomIdToWaitingRoom = new ConcurrentHashMap<>(); // roomId → WaitingRoom
  ...
  public void register(String sessionId, String nickname, String roomId) {
    WaitingRoom room = roomIdToWaitingRoom.computeIfAbsent(roomId, WaitingRoom::new); // room 이 없으면 자동 생성
  ...
  public String getNickName(String sessionId) {
    ...
    WaitingRoom room = roomIdToWaitingRoom.get(roomId);
  ...
  public void unregister(String sessionId) {
    ...
    WaitingRoom room = roomIdToWaitingRoom.get(roomId); // roomId로 대기실 찾음
    room.removeSession(sessionId); // 대기실 내부에서 해당 유저 제거
    if (room.getPlayerCount() == 0) {
      roomIdToWaitingRoom.remove(roomId); // 아무도 안 남으면 방도 제거
    }
  }
  ...
  // 세션이 현재 리더인지 확인
  public boolean isLeader(String sessionId, String roomId) {
    WaitingRoom room = roomIdToWaitingRoom.get(roomId);
    if (room == null) return false;

    return room.getPlayerSet().stream()
        .anyMatch(p -> p.getSessionId().equals(sessionId) && p.isLeader());
  }

  // 게임 시작 처리
  public void startGame(String roomId, int rounds) {
    WaitingRoom room = roomIdToWaitingRoom.get(roomId);
    if (room != null) {
      room.setSelectRounds(rounds);
      room.startGame();
      System.out.println("게임 시작: " + rounds + "라운드");
    }
  }
}
